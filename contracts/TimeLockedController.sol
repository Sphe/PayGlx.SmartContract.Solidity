...
contract TimeLockedController is HasNoEther, HasNoTokens, Claimable {
    ...
    // admin initiates a request to mint _value GlobCoin for account _to
    ...
    // Transfer ownership of a contract from globCoin
    ...
    function changeStakingFees(uint256 _transferFeeNumerator,
        ...
        globCoin.changeStakingFees(_transferFeeNumerator,
    ...
    // Incoming delegate* calls from _source will be accepted by globCoin
    function setDelegatedFrom(address _source) public onlyOwner {
        globCoin.setDelegatedFrom(_source);
    }

    // Update this contract's globCoin pointer to newContract (e.g. if the
    // contract is upgraded)
    function setGlobCoin(GlobCoin _newContract) public onlyOwner {
        emit SetGlobCoin(_newContract);
        globCoin = _newContract;
    }

    // change globCoin's name and symbol
    ...
    // Swap out GlobCoin's permissions registry
