...
contract('GlobCoin', function (accounts) {
    const [_, owner, oneHundred, anotherAccount] = accounts
    const notes = "some notes"

    describe('--GlobCoin Tests: 1 contract--', function () {
        beforeEach(async function () {
            // Set up a GlobCoin contract with 100 tokens for 'oneHundred'.
            this.registry = await Registry.new({ from: owner })
            this.balances = await BalanceSheet.new({ from: owner })
            this.allowances = await AllowanceSheet.new({ from: owner })
            this.token = await GlobCoin.new({ from: owner })
    ...
    describe('--GlobCoin Tests: chaining 2 contracts--', function () {
        const _ = accounts[0]
        ...
        beforeEach(async function () {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                this.tokens[i] = await GlobCoinMock.new(oneHundreds[i], 100, { from: owners[i] })
    ...
    describe('--GlobCoin Tests: chaining 3 contracts--', function () {
        const _ = accounts[0]
        ...
        beforeEach(async function () {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                this.tokens[i] = await GlobCoinMock.new(oneHundreds[i], 100*10**18, { from: owners[i] })
                await this.tokens[i].setRegistry(this.registries[i].address, { from: owners[i] })
            }
        })
