...
const GlobCoin = artifacts.require("GlobCoin")
const BalanceSheet = artifacts.require("BalanceSheet")
const AllowanceSheet = artifacts.require("AllowanceSheet")
const TimeLockedController = artifacts.require("TimeLockedController")
const GlobCoinMock = artifacts.require("GlobCoinMock")
...
contract('TimeLockedController', function (accounts) {
    describe('--TimeLockedController Tests--', function () {
        ...
        beforeEach(async function () {
            ...
            this.token = await GlobCoinMock.new(oneHundred, 100*10**18, { from: owner })
            ...
            await this.controller.setGlobCoin(this.token.address, { from: owner })
    ...
    describe('--TimeLockedController old test--', function () {
        it("should work", async function () {
            ...
            const GlobCoin = await GlobCoin.new()
            await balances.transferOwnership(GlobCoin.address)
            await allowances.transferOwnership(GlobCoin.address)
            await GlobCoin.setBalanceSheet(balances.address)
            await GlobCoin.setAllowanceSheet(allowances.address)
            ...
            await GlobCoin.transferOwnership(timeLockedController.address, { from: accounts[0] })
            await timeLockedController.issueClaimOwnership(registry.address, { from: accounts[0] })
            await timeLockedController.issueClaimOwnership(GlobCoin.address, { from: accounts[0] })
            ...
            await assertRevert(GlobCoin.mint(accounts[3], 10, { from: accounts[0] })) //user 0 is no longer the owner
            ...
            await assertBalance(GlobCoin, accounts[3], 9)
            ...
            await assertBalance(GlobCoin, accounts[3], 209)
            ...
            await assertBalance(GlobCoin, accounts[3], 40209)
            ...
            await assertRevert(timeLockedController.transferChild(GlobCoin.address, accounts[2], { from: accounts[1] })) //only owner
            await timeLockedController.requestMint(accounts[3], 500000, { from: accounts[2] })
            await timeLockedController.transferChild(GlobCoin.address, accounts[2], { from: accounts[0] })
            await timeLockedController.transferChild(registry.address, accounts[2], { from: accounts[0] })
            await GlobCoin.claimOwnership({ from: accounts[2] })
            await assertRevert(timeLockedController.finalizeMint(4, { from: accounts[2] })) //timeLockedController is no longer the owner of GlobCoin
            await GlobCoin.transferOwnership(timeLockedController.address, { from: accounts[2] })
            await timeLockedController.issueClaimOwnership(GlobCoin.address, { from: accounts[0] })
            await increaseTime(duration.days(1))
            await timeLockedController.finalizeMint(4, { from: accounts[2] })
            await assertBalance(GlobCoin, accounts[3], 540209)
        })
    })
})
